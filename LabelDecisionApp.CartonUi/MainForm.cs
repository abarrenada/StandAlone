...
public class MainForm : Form
{
    ...
    private void BuildStartupPanel()
    {
        ...
        // Shift
        _startupPanel.Controls.Add(MakeLabel("Shift:", new Point(labelX, y + 4), labelW));
        _shiftInput = new NumericUpDown
        {
            Location = new Point(inputX, y), Size = new Size(80, 30),
        ...
        // Inspector
        _startupPanel.Controls.Add(MakeLabel("Inspector:", new Point(labelX, y + 4), labelW));
        _inspectorInput = new TextBox
        {
            Location = new Point(inputX, y), Size = new Size(inputW, 30),
        ...
        // Label Size
        _startupPanel.Controls.Add(MakeLabel("Label Size:", new Point(labelX, y + 4), labelW));
        ...
        // Side-by-side size reference list (like Progress 2-column aSzPrmpt display)
        // Position below the combo, not overlapping buttons
        var sizeListLabel = new Label
        {
            Location = new Point(inputX, y + 36),
            Size = new Size(600, 120),
            ForeColor = Color.LightCyan,
            Font = new Font("Courier New", 8.5f),
            Text = BuildLabelSizeListText(),
        };
        _startupPanel.Controls.Add(sizeListLabel);
        y += rowStep + 50;
        if (_config.DoesManStk && !_config.DoesMexico)
        {
            _startupPanel.Controls.Add(MakeLabel("Primary Item:", new Point(labelX, y + 4), labelW));
        ...
        // 2nd Primary Item (DoesTwoPrims + DoesManStk + not Mexico)
        if (_config.DoesTwoPrims && _config.DoesManStk && !_config.DoesMexico)
        {
            _startupPanel.Controls.Add(MakeLabel("2nd Primary Item:", new Point(labelX, y + 4), labelW));
        ...
        // Begin button — anchored to bottom-left
        _btnBegin = new Button
        {
            Text = "Begin  [Enter]",
            Size = new Size(160, 44),
            Anchor = AnchorStyles.Bottom | AnchorStyles.Left,
            Location = new Point(inputX, _startupPanel.Height - 64),
        ...
        var btnExit = new Button
        {
            ...
            Anchor = AnchorStyles.Bottom | AnchorStyles.Left,
            Location = new Point(inputX + 176, _startupPanel.Height - 64),
    ...
    private async Task ExportLabelToNiceLabelXmlAsync(BoxRecord box, StackerRecord? stacker, string itemDisplay, CancellationToken ct)
    {
        try
        {
            ...
            if (_settings.LabelOutputType != "NiceLabel Xml")
                return;
